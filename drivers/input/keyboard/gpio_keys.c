...
static bool vol_up_pressed = false;
...
#ifdef CONFIG_GPIO_KEYS_TRIGGER_PANIC
    if (button->code == KEY_VOLUMEUP && trigger_panic) {
        if (state) { /*key pressed*/
            vol_up_pressed = true;
            panic_start = jiffies;
            printk(KERN_INFO "Key: gpio %d, code %d pressed at %ul\n",
                    button->gpio, button->code, panic_start);
        } else { /* key released */
            bool is_timeout = false;
            unsigned long now = jiffies;
            printk(KERN_INFO "Key: gpio %d, code %d released at %ul, (start = %ul)\n",
                    button->gpio, button->code, now, panic_start);
            if (!vol_up_pressed) {
                printk(KERN_INFO "Key: gpio %d, code %d press event arrived "
                        "before press\n", button->gpio, button->code);
                goto no_panic;
            }
            is_timeout = time_after(now, (panic_start + msecs_to_jiffies(10000)));
            if (is_timeout && power_button_pressed) {
                pr_err("Panic triggered because of [Vol up] and [Power button]:\n");
                dump_stack();
                BUG();
                while(true);
            }
            vol_up_pressed = false;
            panic_start = 0xffffffff; /*Clean up the start jiffies */
        }
    }
no_panic:
#endif

...
#define CONFIG_AREA_BIT_OFFSET 5

#define MASK_FLASH_STATUS  0x07
#define MASK_FW_UPDATE_CMD 0x0f
...
static int fwu_read_f34_queries(void)
{
	...
	if (fwu->flash_properties & HAS_DISP_CONFIG) {
		...
		count += sizeof(fwu->disp_config_block_count);
	}
...
static int fwu_read_f34_flash_status(void)
{
	...
	if (fwu->bl_version == V5)
		fwu->flash_status = (status >> 4) & MASK_FLASH_STATUS;
	else if (fwu->bl_version == V6)
		fwu->flash_status = status & MASK_FLASH_STATUS;
	...
	fwu->command = command & MASK_FW_UPDATE_CMD;
...
{
	int retval;
	unsigned char command = cmd & MASK_FW_UPDATE_CMD;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
...
static int fwu_scan_pdt(void)
{
	...
	for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
		...
		intr_count += (rmi_fd.intr_src_count & MASK_INT_SRC_CNT);
	}
...
static int fwu_do_read_config(void)
{
	...
	block_offset[1] |= (fwu->config_area << CONFIG_AREA_BIT_OFFSET);

...
static ssize_t synaptics_rmi4_debug_switch(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(debug_switch, S_IWUSR | S_IWGRP,
			synaptics_rmi4_show_error,
			synaptics_rmi4_debug_switch),
	__ATTR(ic_info, 0444,
...
static ssize_t synaptics_rmi4_glove_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned char command_off = 0x02;
	unsigned char command_on = 0x03;
	unsigned int debug_control;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);

	if (sscanf(buf, "%u", &debug_control) != 1)
		return -EINVAL;
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "synaptics_%s_%s_%d\n",rmi4_data->rmi4_mod_info.product_id_string, vendor_id, SYNA_FW_VERSION);
}

static ssize_t synaptics_rmi4_sensorinfor_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "G/MSensor_ST_LSM303d\nLSensor_LSC_AL3320\n");
}
...
//			if (rmi4_data->hw_if->board_data->x_flip)
			...
			if(touch_debug)
				printk(KERN_DEBUG "Finger f12 %d: x = %d, y = %d.\n", finger, x, y);
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	dev_dbg(rmi4_data->pdev->dev.parent, "%s, touch_stop=%d\n", __func__, rmi4_data->touch_stopped);
		...
		bool enable)
{
	...
	dev_err(rmi4_data->pdev->dev.parent, "%s, irq=%d, enable=%d\n", __func__, rmi4_data->irq, enable);
		...
		if (retval < 0)
			return retval;
                ...
                retval = request_threaded_irq(rmi4_data->irq, NULL,
                                synaptics_rmi4_irq, bdata->irq_type,
...
#define PMIC_GPIO1P3_PWR  (105)

	//intel_scu_ipc_msic_vprog2(1);
	//printk("enable vprog2 for sensor\n");
	retval = gpio_request(PMIC_GPIO1P3_PWR, "pmic_gpio1p3_pwr");			

        if (retval) {
             pr_err("%s: Failed to get gpio %d (code: %d)",
                                       __func__, PMIC_GPIO1P3_PWR, retval);
             return retval;
        }

	retval = gpio_direction_output(PMIC_GPIO1P3_PWR, 1);	

	msleep(50);

        if (retval) {
                        pr_err("%s: Failed to set gpio %d direction",
                                        __func__, PMIC_GPIO1P3_PWR);
                        return retval;
        }

	printk("%s, configure PMIC_GPIO1P3_PWR output as 1 to enable V3P3S_TCH\n", __func__);

	...
	if (data && 0 == strncmp(cmd, "recovery", 9)) {
		...
		if (ret_ipc < 0)
			pr_err("%s cannot write Fastboot reboot reason in OSNIB\n",
				__func__);
		ret = NOTIFY_OK;
	} else if (data && 0 == strncmp(cmd, "testmode", 9)) {
		pr_warn("[SHTDWN] %s, rebooting into Testmode\n", __func__);
		ret_ipc = intel_scu_ipc_write_osnib_rr(SIGNED_TEST_ATTR);
		if (ret_ipc < 0)
			pr_err("%s cannot write Testmode reboot reason in OSNIB\n",
				__func__);
		ret = NOTIFY_OK;

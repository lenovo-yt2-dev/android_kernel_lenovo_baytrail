...
#define OV8865_DEFAULT_LOG_LEVEL 0

static int log_level = OV8865_DEFAULT_LOG_LEVEL;
...
ov8865_read_reg(struct i2c_client *client, u16 len, u16 reg, u16 *val)
{
	...
	if (len > OV8865_BYTE_MAX) {
       ...
       printk("ov8865_read_reg client->addr %x\n ",client->addr);
...
static int bu64243_write8(struct v4l2_subdev *sd, int reg, int val)
{
	...
	OV8865_LOG(0, "%s %d reg:0x%x val:0x%x\n", __func__, __LINE__, reg, val);
...
static int bu64243_cmd(struct v4l2_subdev *sd, s32 reg, s32 val)
{
	...
	OV8865_LOG(0, "BU64243_PS(dev->isrc):%x-->%x\n", dev->isrc_mode, BU64243_M(dev->isrc_mode));
	OV8865_LOG(0, "BU64243_PS(dev->D9D8):%x-->%x\n", val, BU64243_D_HI(val));
...
static int bu64243_init(struct v4l2_subdev *sd)
{
	...
	OV8865_LOG(0, "Bu64243 needs no platform data, nothing need to do\n");
	return 0;
}
...
static int ov8865_detect(struct i2c_client *client, u16 *id, u8 *revision)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_I2C)) {
		printk("%s %d I2C access enable issue \n", __func__, __LINE__);
		return -ENODEV;
	}
	OV8865_LOG(0, "%s %d\n", __func__, __LINE__);
			    ...
			    int irq, void *pdata)
{
	...
	OV8865_LOG(0, "%s %d ov8865 platform_init done\n", __func__, __LINE__);
	...
	OV8865_LOG(0, "%s %d ov8865 s_power done\n", __func__, __LINE__);
	dev->otp_data = NULL;
	...
	if (ret) {
		...
		if (!ret) {
			OV8865_LOG(0, "ov8865 otp read done\n");
	...
	if (ret)
		goto fail_csi_cfg;
	OV8865_LOG(0, "%s %d ov8865 csi_cfg done\n", __func__, __LINE__);
	...
	OV8865_LOG(0, "%s %d ov8865 detect done\n", __func__, __LINE__);
	...
	OV8865_LOG(0, "%s %d ov8865 s_config successfully\n", __func__, __LINE__);
	return 0;
			 ...
			 const struct i2c_device_id *id)
{
	...
	OV8865_LOG(0, "%s %d s_config done\n", __func__, __LINE__);
	...
	OV8865_LOG(0, "%s %d handle init done\n", __func__, __LINE__);

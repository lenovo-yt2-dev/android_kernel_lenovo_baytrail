...
static int ov9760_dump_otp(const char *val, struct kernel_param *kp)
{
	...
	if(ret != 0)
		OTP_LOG("Fail to save ov9760 RAW data\n");

	ret = ov9760_otp_save(otp_data,otp_size,OV9760_SAVE_OTP_DATA);
	if(ret != 0)
		OTP_LOG("Fail to save ov9760 OTP data\n");
	return 0;
}
				...
				struct camera_mipi_info *info)
{
	...
	printk("reg[0x30b0..30b4]: 0x%x 0x%x 0x%x 0x%x 0x%x\n", reg_0x30b0,
			reg_0x30b1,reg_0x30b2,reg_0x30b3,reg_0x30b4);
	printk("vt_pix_clk_div:0x%x vt_sys_clk_div:0x%x pll_multiplier:0x%x \
...
ov9760_s_config(struct v4l2_subdev *sd, int irq, void *platform_data)
{
	...
	if(ret) {
		printk("ov9760 load from user-space failed, load from sensor\n");

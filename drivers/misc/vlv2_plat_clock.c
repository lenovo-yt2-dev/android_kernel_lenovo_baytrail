...
int vlv2_plat_configure_clock(int clk_num, u32 conf)
{
	...
	pr_err("vlv2_plat_configure_clock(), clk %d, status: %d\n", clk_num, vlv2_plat_get_clock_freq(clk_num));
	return 0;
}
...
void vlv2_print_all_clock_status()
{
	int clk_num;
	printk("before sleep, dump the clock status:\n");
	for(clk_num = 0;clk_num<=MAX_CLK_COUNT;clk_num++){
		printk("    Platform Clock %d: %s\n",clk_num,(vlv2_plat_get_clock_status(clk_num)==1)? "on" : "off");
	}
	printk("dump done!\n");
}
EXPORT_SYMBOL_GPL(vlv2_print_all_clock_status);
